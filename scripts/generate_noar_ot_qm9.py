...
class PabloEscobar():
    def __init__(self, model):
        ...
        weights = "QM9_noar_ot_all3_09-24_11-05-49_2" #"DRUGS_noar_one_fifth_ot4_09-21_16-58-11_1.pt"
        self.mole_name = "CC_QM9_testset_ot_" + weights  + suffix
        self.name = "CC_QM9_testset"
        self.model = model
        self.weights_path = "/home/dreidenbach/code/mcg/coagulation/scripts/model_ckpt/" + weights + ".pt"
        
        self.true_mols = '/data/dreidenbach/data/torsional_diffusion/QM9/test_mols.pkl'
        with open(self.true_mols, 'rb') as f:
            self.true_mols = pickle.load(f)
        self.valid_mols = '/data/dreidenbach/data/torsional_diffusion/QM9/test_smiles.csv'
        self.test_data = pd.read_csv(self.valid_mols)
        self.types = qm9_types
        self.save_dir = '/data/dreidenbach/data/torsional_diffusion/QM9/test_set'
        self.batch_size = 400
        if False:
    ...
    def clean_confs(self, smi, confs):
        ...
        for i, c in enumerate(confs):
            ...
            if conf_smi == smi:
                good_ids.append(i)
        return [Chem.AddHs(confs[i]) for i in good_ids]
    ...
    def generate(self):
        ...
        with torch.no_grad():
            ...
            # for smi_idx, batch in tqdm(self.dataloader, total=len(self.dataloader)):
                ...
                # print(A_graph.ndata['feat'].shape, B_graph.ndata['feat'].shape, geo_A.num_nodes, geo_B.num_nodes)
                # if count > 62:
                #     import ipdb; ipdb.set_trace()
                ...
                # print("DOG", A_graph.ndata['feat'].shape, B_graph.ndata['feat'].shape)
