...
#     lines = f.readlines()
#     check = set([line.strip() for line in lines])
bad_smile = 'C#CCOCCOCCOCCNc1nc(N2CCN(C(=O)[C@H](CCC(=O)O)n3cc([C@H](N)CO)nn3)CC2)nc(N2CCN(C(=O)[C@H](CCC(=O)O)n3cc([C@@H]([NH3+])CO)nn3)CC2)n1'
...
# with open('/data/dreidenbach/data/torsional_diffusion/DRUGS/test_set/drugs_full_gd_rmsd_results_coords.pkl', 'wb') as handle:
...
#     for i_true in range(n_true):
#         # jobs.append((smi, corrected_smi, i_true))
#         smi, correct_smi, i_true = smi, corrected_smi, i_true
#         true_confs = true_mols[smi]
#         model_confs = model_preds[correct_smi]
#         tc = true_confs[i_true]
#         coords = []
...
#                 coords = [np.nan] * len(model_confs)
#                 break
#         results[(smi, correct_smi)]['coords'][i_true] = coords
#         # return smi, correct_smi, i_true, rmsds, coords
...
with open('/data/dreidenbach/data/torsional_diffusion/DRUGS/test_set/drugs_full_cc_noar_rmsd_results_coords_restart3.pkl', 'wb') as handle:
    pickle.dump(results, handle)
    
# import ipdb; ipdb.set_trace()
# test = 1
# python evaluate_confs.py --confs /data/dreidenbach/data/torsional_diffusion/QM9/test_set/CC_QM9_testset_ot_QM9_noar_ot_all3_09-24_11-05-49_2_09-24_19-08-06_generated_molecules.pkl --test_csv /data/dreidenbach/data/torsional_diffusion/QM9/test_smiles.csv --true_mols /data/dreidenbach/data/torsional_diffusion/QM9/test_mols.pkl --n_workers 10 > cc_noar_all_qm9_3.txt