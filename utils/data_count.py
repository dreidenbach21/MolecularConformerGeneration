...
class ConformerDataset(DGLDataset):
    def __init__(self, root, split_path, mode, types, dataset, num_workers=1, limit_molecules=None,
                 ...
                 invariant_latent_dim = 64, equivariant_latent_dim = 32, use_diffusion_angle_def = False, old_rdkit = False, count = False):
...
#         super(ConformerDataset, self).__init__(name, transform) /data/dreidenbach/data
    ...
    def process(self):
        if False: #self.cache_path and os.path.exists(self.cache_path):
    ...
    def preprocess_datapoints_chunk(self, root, split_path, pickle_dir, mode, num_workers, limit_molecules):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            ...
            counts = [len(sublist) for sublist in molecules if sublist is not None and sublist[0] is not None]
...
#                     datapoints.append(t)
    ...
    def filter_smiles_mp(self, smile):
        ...
        if N < 4:
            ...
            return [None]
        # print("A filter")
        edge_mask, mask_rotate = get_transformation_mask(mol, None)
        if np.sum(edge_mask) < 0.5: #TODO: Do we need this since we are using GEOMOL
            self.failures['no_rotable_bonds'] += 1
            return [None]
        check = self.featurize_mol(mol_dic, old_rdkit = self.old_rdkit)
    ...
    def has_cache(self):
        ...
        # else:
        #     return False and os.path.exists(os.path.join(self.save_dir, f'{self.use_name}_0_graphs.bin'))
    ...
    def featurize_mol(self, mol_dic, use_rdkit_coords = False, limit = 5, old_rdkit = False):
        ...
        for idx, conf in enumerate(confs):
            ...
            correct_mol = mol
            mol_features = 1 #featurize_mol(correct_mol, self.types, use_rdkit_coords = use_rdkit_coords, old_rdkit=old_rdkit)
...
def drug_count_local(batch_size = 32, mode = 'train', data_dir='/data/dreidenbach/data/torsional_diffusion/DRUGS/drugs/',
                dataset='drugs', limit_mols=0, log_dir='./test_run', num_workers=16, restart_dir=None, seed=0,
    ...
    data = ConformerDataset(data_dir, split_path, mode, dataset=dataset,
                                   ...
                                   name=f'{dataset}_{mode}_{limit_mols}_final_count',
                                   pickle_dir=std_pickles,
                                   use_diffusion_angle_def=use_diffusion_angle_def,
                                   raw_dir='/data/dreidenbach/data/torsional_diffusion/DRUGS/dgl', 
                                   save_dir='/data/dreidenbach/data/torsional_diffusion/DRUGS/dgl',
                                   boltzmann_resampler=None,
                                   count = True)
    
    # dataloader = None #dgl.dataloading.GraphDataLoader(data, use_ddp=False, batch_size=batch_size, shuffle=True, drop_last=False, num_workers=num_workers,
                                            # collate_fn = collate)
    return data.counts, data

def qm9_count_local(batch_size = 32, mode = 'train', data_dir='/data/dreidenbach/data/torsional_diffusion/QM9/qm9/',
                dataset='drugs', limit_mols=0, log_dir='./test_run', num_workers=16, restart_dir=None, seed=0,
    ...
    data = ConformerDataset(data_dir, split_path, mode, dataset=dataset,
                                   ...
                                   pickle_dir=std_pickles,
                                   use_diffusion_angle_def=use_diffusion_angle_def,
                                   raw_dir='/data/dreidenbach/data/torsional_diffusion/QM9/dgl', 
                                   save_dir='/data/dreidenbach/data/torsional_diffusion/QM9/dgl',
                                   boltzmann_resampler=None,
                                   count = True)
