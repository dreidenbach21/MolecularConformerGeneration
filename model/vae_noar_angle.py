...
class VAENoArAngle(nn.Module):
    def __init__(self, kl_params, encoder_params, decoder_params, loss_params, coordinate_type, device = "cuda"):
        ...
        self.angle_predictor = nn.Sequential(Scalar_MLP(angle_feat_dim, 2*angle_feat_dim, 2*angle_feat_dim, use_batchnorm = True),
                                             Scalar_MLP(2*angle_feat_dim, 2*angle_feat_dim, 2*angle_feat_dim, use_batchnorm = True),
                                             Scalar_MLP(2*angle_feat_dim, angle_feat_dim, 1, use_batchnorm = True))
    ...
    def angle_forward(self, generated_molecule, angles, angles_mask, frag_ids):
        ...
        print("raw angle deltas:", angle_deltas[:20].squeeze(1))
        ...
        angle_deltas = angle_deltas.squeeze(1)
        ...
        print("clamped/scale angle deltas:", angle_deltas[:20])
    ...
    def loss_function(self, generated_molecule, rdkit_reference, channel_selection_info, KL_terms, enc_out, geometry_graph, angles, log_latent_stats = True):
        ...
        if log_latent_stats:
            loss_results = {
                ...
                'angle_regression_loss': angle_regression_loss.item(),
