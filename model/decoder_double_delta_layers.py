...
class IEGMN_Bidirectional_Double_Delta_Layer(nn.Module):
    def __init__(
            self,
        ...
        super(IEGMN_Bidirectional_Double_Delta_Layer, self).__init__()
        ...
        self.weight_sharing = True #weight_sharing #! For now we are removing the B weights since they dont do much for TF
        ...
        if self.use_dist_in_layers:# and self.A_evolve: #TRUE and TRUE
            A_edge_mlp_input_dim += 3*len(self.all_sigmas_dist)
        ...
        if self.weight_sharing:
            ...
            if self.use_dist_in_layers:# and self.B_evolve:
                B_edge_mlp_input_dim += 3*len(self.all_sigmas_dist)
    ...
    def apply_edges_A(self, edges):
        if self.use_dist_in_layers:# and self.A_evolve:
            ...
            x_rel_ref_mag = edges.data['x_rel_ref_cc'] ** 2
            ...
            self_ref_mag = (edges.src['x_now_cc'] - edges.src['x_ref']) ** 2
            ...
            return {'msg_cc': self.A_edge_mlp(
                torch.cat([edges.src['feat_cc'], edges.dst['feat_cc'], x_rel_mag, x_rel_ref_mag, self_ref_mag], dim=1))} # edges.data['feat'], x_rel_mag], dim=1))} # operates with edge features in it and node features
        else:
            assert(1 == 0)
            return {
    ...
    def update_x_moment_A(self, edges):
        # TODO: Do we want to change this to some how incorporate the ref?
    ...
    def single_forward(self, A_graph, coords_A, h_feats_A, original_A_node_features, orig_coords_A, geometry_graph_A = None, mask = None):
        with A_graph.local_scope():
            ...
            A_graph.apply_edges(fn.u_sub_v('x_now_cc', 'x_ref', 'x_rel_ref_cc'))
            # A_graph.ndata['x_rel_ref_self_cc'] = A_graph.ndata['x_now_cc'] - A_graph.ndata['x_ref']
            ...
            # TODO: try still adding prior in an intuitive way. TRY THE ABOVE BUT INSTEAD OF NOW WE DO X_REF
            x_evolved_A = A_graph.ndata['x_ref'] + A_graph.ndata['x_update_cc']
            # x_evolved_A = A_graph.ndata['reference_point'] + self.double_net(x_evolved_A - A_graph.ndata['reference_point'])
            # else:
    ...
    def forward(self, A_graph, B_graph, coords_A, h_feats_A, original_A_node_features, orig_coords_A,
        ...
        with A_graph.local_scope() and B_graph.local_scope():
            ...
            # B_graph.apply_edges(fn.u_sub_v('x_now_cc', 'x_ref', 'x_rel_ref_cc'))
            ...
            # x_evolved_B = B_graph.ndata['x_ref'] + B_graph.ndata['x_update_cc']
            ...
            node_upd_B = h_feats_B #B_graph.ndata['feat_cc']
            # print("   [DEC MPNN] feat A", torch.min(node_upd_A).item(), torch.max(node_upd_A).item(), torch.norm(node_upd_A, 2).item())
...
class IEGMN_Bidirectional_Double_Delta(nn.Module):
    def  __init__(self, n_lays, debug, device, shared_layers, noise_decay_rate, cross_msgs, noise_initial,
        ...
        super(IEGMN_Bidirectional_Double_Delta, self).__init__()
        self.debug = debug
        self.cross_msgs = cross_msgs
        self.device = device
        self.save_trajectories = save_trajectories
        ...
        self.iegmn_layers.append(
            IEGMN_Bidirectional_Double_Delta_Layer(orig_invar_feats_dim_h=input_node_feats_dim,
        ...
        # Create N - 1 layers
        if shared_layers:
            interm_lay = IEGMN_Bidirectional_Double_Delta_Layer(orig_invar_feats_dim_h=input_node_feats_dim,
