...
class ECN(nn.Module):
    def __init__(self, n_lays, debug, device, shared_layers, noise_decay_rate, cross_msgs, noise_initial,
                 use_edge_features_in_gmn, use_mean_node_features, atom_emb_dim, latent_dim,
        ...
        self.pooling_layers.append(
            Pooling_Layer(invar_feats_dim_h=latent_dim,
                        out_feats_dim_h=latent_dim,
                        nonlin=nonlin,
                        cross_msgs=self.cross_msgs,
                        leakyrelu_neg_slope=leakyrelu_neg_slope,
                        ...
                        weight_sharing = self.weight_sharing, **kwargs))
        if shared_layers:
            interm_lay = Pooling_Layer(invar_feats_dim_h=latent_dim,
                                     out_feats_dim_h=latent_dim,
                                     cross_msgs=self.cross_msgs,
                                     nonlin=nonlin,
                                     leakyrelu_neg_slope=leakyrelu_neg_slope,
        ...
        self.cg_layers.append(
            Coarse_Grain_Layer(invar_feats_dim_h=latent_dim,
                        out_feats_dim_h=latent_dim,
                        nonlin=nonlin,
                        cross_msgs=self.cross_msgs,
                        leakyrelu_neg_slope=leakyrelu_neg_slope,
        ...
        # Create N - 1 layers
        if shared_layers:
            interm_lay = Coarse_Grain_Layer(invar_feats_dim_h=latent_dim,
                                     out_feats_dim_h=latent_dim,
                                     cross_msgs=self.cross_msgs,
                                     nonlin=nonlin,
                                     leakyrelu_neg_slope=leakyrelu_neg_slope,
    ...
    def forward(self, A_graph, B_graph, geometry_graph_A, geometry_graph_B, A_pool, B_pool,
                  A_cg, B_cg, geometry_graph_A_cg, geometry_graph_B_cg, epoch):
        ...
        for i, layer in enumerate(self.fine_grain_layers):
            ...
            # Pooling Layer
            coords_A_pool, h_feats_A_pool, coords_B_pool, h_feats_B_pool = self.pooling_layers[i](A_pool=A_pool,
                            B_pool = B_pool,
                            fine_h_A=h_feats_A,
                            fine_h_B=h_feats_B,
            ...
            h_feats_A_cg, \
            coords_B_cg, \
            h_feats_B_cg, trajectory_cg, geom_loss_cg = self.cg_layers[i](A_graph=A_cg,
                                B_graph=B_cg,
                                coords_A=coords_A_cg,
                                h_feats_A=h_feats_A_cg,
                                orig_coords_A=orig_coords_A_cg,
                                coords_B=coords_B_cg,
                                h_feats_B=h_feats_B_cg,
                                orig_coords_B=orig_coords_B_cg,
                                mask=coarse_mask,
