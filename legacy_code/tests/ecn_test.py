...
if __name__ =="__main__":
    ...
    print("# of CG Params = ", sum(p.numel() for name, p in model.named_parameters() if p.requires_grad and "cg_layers" in name))
    if True:
        for name, param in model.named_parameters():
            if param.requires_grad:
                print( name, param.shape, param.numel())
    (v_A_cg, h_feats_A_cg), (v_B_cg, h_feats_B_cg), geom_losses, geom_loss_cg, full_trajectory, full_trajectory_cg = model(A, B,
        geometry_graph_A, geometry_graph_B,
        Ap, Bp, A_cg, B_cg, geometry_graph_A_cg, geometry_graph_B_cg, epoch=0)
    print(v_A_cg.shape, h_feats_A_cg.shape)
    print("Test Success!")
