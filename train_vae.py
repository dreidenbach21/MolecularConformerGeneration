...
def dummy_data_loader():
  smiles = ['CC[C@@](C)(O)[C@@H](C)OC', 'C[C@@]12COCC=C[C@@H]1C2', 'C[C@@]12C[C@H]3CN1[C@]32CO']
  data = []
  frag_ids = []
  # bond_breaks = []
  for smi in smiles:
...
if __name__ =="__main__":
  ...
  optim = torch.optim.AdamW(model.parameters(), lr=1e-3)#3) #! 1e-2 was giving us nan blow ups look into gradient clipping
  ...
  loss_log_name = "train_vae_losses_test_1k_v4"
  loss_log = []
  for epoch in range(10000):
    ...
    #   ipdb.set_trace() #! we are seeing blow ups of the last molecule
    #   generated_molecule, rdkit_reference, dec_results, KL_terms, enc_out = model(frag_ids, A_batch, B_batch, geoA_batch, geoB_batch, Ap_batch, Bp_batch, Acg_batch, Bcg_batch, geoAcg_batch, geoBcg_batch, epoch=epoch)
    #   loss, losses = model.loss_function(generated_molecule, rdkit_reference, dec_results, KL_terms, step = epoch)
    print(f"Loss = {loss}")
    loss.backward()
